...
import { setComponentIndex, setComponentVisibility, setComponentsFromLocalStorage } from '../redux/slices/settings';
...
export default () => {
	...
	const setVisibility = useCallback(({ name }) => {
		dispatch(setComponentVisibility({ name, visible: !components[name].visible }));
		setComponents({ ...components, [name]: { ...components[name], visible: !components[name].visible }});
	}, [components, dispatch]);
	const calculateIndex = useCallback(({ movement }) => {
		...
		if (index <= 0)
			return 1;
		if (index > Object.keys(components).length)
	...
	useEffect(() => {
		const handleMouseUp = (e) => {
			if (dragItem.active) {
				...
				const hoverItemName = Object.keys(components).find(key => components[key].index === index);
				const hoverItemIndex = Object.values(components).find(item => item.name === dragItem.name).index;
				const newComponents = {
					...components,
					[dragItem.name]: {
