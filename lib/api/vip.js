...
export const getVipData = async ({ word }) => {
	const response = await axios.get(`http://vipingilizce.net/kelime/${word}/?tumunu_goster=ok`);
	const dom = new JSDOM(response.data);
	const document = dom.window.document;
	if (document.querySelector('.HataMesaj') !== null) return [];
	else {
		let sentencesNodes = Array.from(document.querySelectorAll('.Cumleler > p'));
		sentencesNodes = sentencesNodes.concat(
			Array.from(document.querySelectorAll('#divDigerCumleler > p')),
		);
		return sentencesNodes.map((sentenceNode) => {
			...
			Array.from(sentenceNode.childNodes).forEach((node) => {
				if (node.nodeName === 'BR') {
					...
					if (brIsPassed) translation += node.textContent;
